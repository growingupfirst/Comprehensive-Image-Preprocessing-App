...
def convert_coco_json(json_dir="../coco/annotations/", use_segments=False, cls91to80=False):
    ...
    for json_file in sorted(Path(json_dir).resolve().glob("*.json")):
        ...
        # Write labels file
        for img_id, anns in stqdm(imgToAnns.items(), desc=f"Annotations {json_file}"):
            img = images["%g" % img_id]
...
if sbm_btn:
    with st.spinner('Converting...'):
        convert_coco_json(
                file_path,  # directory with *.json
                use_segments=False,
                cls91to80=False,
            )
    st.success('Convert successfull!')
open_folder = st.button('Open Folder')
if open_folder:
    subprocess.Popen(f'explorer /select,"C:\\Users\\user\\Desktop\\SA\\Comprehensive Image Preprocessing WebApp\\"')